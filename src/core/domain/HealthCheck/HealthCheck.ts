...
export default class HealthCheck {
  ...
  constructor(id: Uuid, endpointId: EndpointId, host: string, address: string, time: number, createdAt: Date) {
    this.id = id
  ...
  static create(endpointId: EndpointId, host: string, address: string, time: number) {
    ...
    return new HealthCheck(Uuid.generate(), endpointId, host, address, time, new Date());
  }
