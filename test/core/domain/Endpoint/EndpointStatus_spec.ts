...
describe('EndpointStatus entity', () => {
  ...
  it('should generate an id', () => {
    const endpoint1 = EndpointStatus.create(userId, url, endpointName);
    const endpoin2 = EndpointStatus.create(userId, url, endpointName);
    should(endpoint1.getId()).be.not.eql(endpoin2.getId());
  });

  it('should fail if the userId is missing', () => {
    should.throws(() => {
      EndpointStatus.create(null, url, endpointName);
    })
  });

  it('should fail if the host is missing', () => {
    should.throws(() => {
      EndpointStatus.create(userId, null, endpointName);
    })
  });
  ...
  context('Update from a health check', () => {
    it('should add a health check information', () => {
      const endpoint = EndpointStatus.create(userId, url, endpointName);
    ...
    it('should keep the latest 50 health checks', () => {
      ...
      const endpoint = new EndpointStatus(randomEndpointId, userId, url, endpointName, new Date(), initialLastChecks, new Date(), 0, null);
      endpoint.updateWithHealthCheck(new EndpointUpdatedEventData('id', 'ip', 'host', 123, new Date()));
    ...
    it('should increase the downtime minutes after the first fail', () => {
      ...
      const endpoint = new EndpointStatus(randomEndpointId, userId, url, endpointName, new Date(), [], new Date(), initialMinutesDown, lastFailedDate);
  ...
  context('Availability calculation', () => {
    ...
    it('should calculate the availability from the first health check', () => {
      ...
      const endpoint = new EndpointStatus(randomEndpointId, userId, url, endpointName, new Date(), [], from, downtimeMinutes, null);
      const availability = endpoint.getAvailability();
